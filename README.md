...
現在は非公開ベータ想定のため、ログインはメールアドレス＋パスワード方式です。自由な新規登録導線はまだ表示していません。
...
OTP / メールリンクログインは、SMTP送信設定と到達性を整備した後で再調整予定です。
...
1. `/login` でメールアドレスとパスワードを入力してログインします。
2. ホームから「今日のトレーニングを記録する」を開きます。
3. `ベンチプレス`、`100kg`、`8回` のセットを保存します。
4. AI診断画面に遷移し、診断が生成されることを確認します。
5. もう一度同じ種目を入力し、前回同種目ログが表示されることを確認します。
