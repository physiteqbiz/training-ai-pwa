...
- `/progress` の進捗可視化（種目別PR、推定1RM推移、週別ボリューム、部位別集計）
...
`/progress` では、保存済みの `workout_sessions` と `workout_sets` を使って進捗を表示します。新しいDBテーブルや集計保存カラムは追加せず、画面表示時に都度集計します。OpenAI APIは呼び出しません。

Freeユーザーは、直近の種目別PRの一部、直近3回分の推定1RM推移、直近7日のボリュームプレビューを確認できます。Proユーザーは、全期間または30日/90日の期間切替で、種目別PR、推定1RM履歴、簡易グラフ、週別ボリューム、部位別セット数・ボリュームを確認できます。

推定1RMは既存のEpley式ヘルパーを使い、1回セットは重量そのものとして扱います。アップ、ドロップ、補助ありセットは推定1RM PRの主要評価から除外します。重量はDBではkg正本のまま保持し、画面表示時にユーザー設定のkg/lbへ変換します。
...
非公開ベータ投入前の詳細な確認項目は [`docs/private-beta-checklist.md`](docs/private-beta-checklist.md) にまとめています。
