...
function buildProgressHighlightFallback(currentAnalysis: CurrentTrainingAnalysis) {
  ...
  const topPerformance = exercise?.top_set ?? exercise?.top_single;

  if (!exercise || !topPerformance || !exercise.main_set) {
    return null;
  }
  ...
  const topPerformanceText = topPerformance.reps === 1
    ? `トップシングル${formatWeight(topPerformance.weight, exercise.weight_unit)}×1回`
    : `トップセット${formatWeight(topPerformance.weight, exercise.weight_unit)}×${topPerformance.reps}回`;
  const strengthText = topPerformance.reps === 1
    ? "単発の強さ"
    : "高重量帯の出力";

  return `${exercise.exercise_name}は${topPerformanceText}で高重量への適応を確認し、メインセットでは${repeatedText}しました。推定1RMの最高値が別セット由来でも、${strengthText}とメイン重量帯の反復性能の両方が安定しています。`;
}
...
function shouldUseProgressHighlightFallback(
  ...
  const topPerformanceWeight = formatWeight(topPerformance.weight, exercise.weight_unit);
  const mentionsTopPerformance =
    text.includes(topPerformanceWeight) || /トップセット|トップシングル|シングル|単発|高重量/.test(text);
  const mentionsMainPerformance = /反復性能|再現性|メイン重量帯|メインセット/.test(text);

  return !text || !mentionsTopPerformance || !mentionsMainPerformance;
}
...
function buildPracticalCaution(currentAnalysis: CurrentTrainingAnalysis) {
  ...
  const topPerformance = exercise.top_set ?? exercise.top_single;
  const topPerformanceText = topPerformance
    ? topPerformance.reps === 1
      ? `${formatWeight(topPerformance.weight, exercise.weight_unit)}のシングル後に`
      : `${formatWeight(topPerformance.weight, exercise.weight_unit)}×${topPerformance.reps}回のトップセット後に`
    : "";
  ...
  const setupText = `${exercise.exercise_name}は${topPerformanceText}${mainWeightText}×${exercise.main_set.reps}回を重ねる構成です。`;
...
function buildExerciseQualityContext(
  ...
  return currentAnalysis.exercises_summary.map((exercise) => {
    ...
    return {
      ...
      top_set_estimated_1rm_kg: exercise.top_set_estimated_1rm_kg,
      top_set_notes: exercise.top_set_notes,
